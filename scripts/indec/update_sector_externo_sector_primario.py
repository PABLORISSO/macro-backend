...
OUT_FILE = BASE_DIR / "src" / "data" / "sector_externo" / "ingreso_primario.csv"
...
def buscar_fila_exacta_inicio(df: pd.DataFrame, texto_inicio: str) -> int:
    texto_inicio = texto_inicio.lower()

    for idx, row in df.iterrows():
        txt = texto_fila(row)
        if txt.startswith(texto_inicio):
            return idx

    raise RuntimeError(f"No se encontró fila que empiece con: {texto_inicio}")
...
def normalizar_ingreso_primario_desde_xls(file_path: Path) -> pd.DataFrame:
    print("Leyendo Cuadro 5 - Ingreso primario...")
    ...
    fila_saldo = buscar_fila_por_claves(df, ["1.b", "ingreso primario"])
    fila_ingresos = buscar_fila_exacta_inicio(df, "crédito")
    fila_egresos = buscar_fila_exacta_inicio(df, "débito")
...
def merge_incremental(new_data: pd.DataFrame, output_path: Path) -> pd.DataFrame:
    ...
    print("Resumen incremental Sector Externo - Ingreso primario:")
...
def main() -> None:
    ...
    df_new = normalizar_ingreso_primario_desde_xls(RAW_FILE)
    df_final = merge_incremental(df_new, OUT_FILE)

    df_final.to_csv(OUT_FILE, index=False, encoding="utf-8-sig")

    print("[OK] Sector Externo - Ingreso primario actualizado:")
    print(OUT_FILE)
    print("Últimos datos:")
    print(df_final.tail())
