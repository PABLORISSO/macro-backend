...
OUT_FILE = BASE_DIR / "src" / "data" / "sector_externo" / "servicios.csv"
...
def normalizar_servicios_desde_xls(file_path: Path) -> pd.DataFrame:
    print("Leyendo Cuadro 4 - Servicios...")
    ...
    fila_saldo = buscar_fila_por_claves(df, ["saldo", "servicios"])
    fila_ingresos = buscar_fila_por_claves(df, ["ingresos"])
    fila_egresos = buscar_fila_por_claves(df, ["egresos"])

    print(f"Fila saldo: {fila_saldo}")
    print(f"Fila ingresos: {fila_ingresos}")
    print(f"Fila egresos: {fila_egresos}")
...
def buscar_fila_por_claves(df: pd.DataFrame, claves: list) -> int:
    ...
    print("DEBUG filas con servicios/ingresos/egresos:")
    for idx, row in df.iterrows():
        txt = texto_fila(row)
        if any(k in txt for k in ["servicios", "ingresos", "egresos", "saldo"]):
            print(idx, "=>", txt)
...
def merge_incremental(new_data: pd.DataFrame, output_path: Path) -> pd.DataFrame:
    ...
    print("Resumen incremental Sector Externo - Servicios:")
